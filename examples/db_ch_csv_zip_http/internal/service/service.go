...
func (s service) exportToCsv(fileName string, all []model.HealthData) error {
	...
	for _, data := range all {
		row := []string{
			...
			data.SmokingHistory,
			strconv.FormatFloat(float64(data.BMI), 'f', -1, 64),
			strconv.FormatFloat(float64(data.Hba1cLevel), 'f', -1, 64),
			strconv.FormatFloat(float64(data.BloodGlucoseLevel), 'f', -1, 64),
