...
func NewClickhouseRepository(ctx context.Context, db *sql.DB) *ClickhouseRepository {
	...
	`); err != nil {
		logger.LogFatal(ctx, err, "failed to create event table", nil)
	}
	...
	go func() {
		for record := range recordChan {
			if err := writer.Write(record); err != nil {
				logger.LogWarn(ctx, err, "failed to write record", nil)
			}
		}
	}()
