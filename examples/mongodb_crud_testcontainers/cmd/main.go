...
import (
	"context"
	"flag"
	"github.com/ciazhar/go-start-small/examples/mongodb_crud_testcontainers/internal/person"
...
func main() {
	...
	if err := app.Listen(":" + viper.GetString("application.port")); err != nil {
		logger.LogFatal(context.Background(), err, "fiber failed to start : %v", nil)
	}
}
