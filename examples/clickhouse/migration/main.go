...
func main() {
	...
	db, err := sql.Open("clickhouse", "tcp://localhost:9000?database=your_database&username=your_username&password=your_password")
	if err != nil {
	...
	config := &clickhouse.Config{
		...
		ClusterName:           "your_cluster_name", // Optional
		MigrationsTable:       "schema_migrations",
		MigrationsTableEngine: "TinyLog", // You can set it to MergeTree or any other engine as per your needs
		MultiStatementEnabled: true,
		MultiStatementMaxSize: clickhouse.DefaultMultiStatementMaxSize,
	}

	// Register the ClickHouse instance
	driver, err := clickhouse.WithInstance(db, config)
	if err != nil {
		log.Fatalf("failed to create ClickHouse driver instance: %v", err)
	}

	// Initialize the migrate instance
	m, err := migrate.NewWithDatabaseInstance(
		"file://path/to/migrations", // Replace with the actual path to your migrations directory
		"clickhouse", driver)
	if err != nil {
		log.Fatalf("failed to create migrate instance: %v", err)
	}

	// Run the migrations (example: up)
